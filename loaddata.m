...
for nnn=1:size(sbjlist,1)             % sbjID
    ...
    for ind_tDCS=1:2                       % tDCS_type        
        for ind_cue=1:2                     % Day/CueType
            ...
            info_session=[ind_cue,ind_tDCS,nnn];         % cue type (1: circle; 2no circle); tDCS (1real;2sham); sbjID
...
% #8: type_rule [1,2], 1: 颜色; 2: 箭头
% #9: type_shiftprep [1,2], 1: 规则稳定期；2：规则不稳定期（有提示）
...
% #9: acc/rt_CSE
% #10: type_rule [1,2], 1: 颜色; 2: 箭头                             color
% #11: type_shiftprep [1,2], 1: 规则稳定期；2：规则不稳定期（有提示）  **
% #12: CueType [1,2], 1, with circle; 2, without circle             **
% #13: tDCS type [1,2], 1: real; 2: sham                            sham
% #14: sbjID
